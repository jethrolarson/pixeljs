import { h } from "@fun-land/fun-web";
import { PackIcon } from "../pack";

const renderIcon = (
  { x, y, game, palette }: PackIcon,
  canvas: HTMLCanvasElement,
) => {
  const ctx = canvas.getContext("2d")!;
  for (let col = 0; col < x; col++) {
    for (let row = 0; row < y; row++) {
      const v = game[col * y + row]; // column-major, matches Level.getGame()
      if (v && v !== "0") {
        const hex = palette[parseInt(v, 10) - 1];
        if (hex) {
          ctx.fillStyle = hex;
          ctx.fillRect(col, row, 1, 1);
        }
      }
    }
  }
  return canvas;
};

// TODO remove or better support the null case
export const renderPixelIcon = (
  icon: PackIcon | null,
  sizePx: number,
): HTMLCanvasElement => {
  const x = icon?.x ?? 1;
  const y = icon?.y ?? 1;
  const scale = sizePx / Math.max(x, y);
  const canvas = h("canvas", {
    width: x,
    height: y,
  });
  canvas.style.width = `${x * scale}px`;
  canvas.style.height = `${y * scale}px`;
  canvas.style.imageRendering = "pixelated";

  return icon ? renderIcon(icon, canvas) : canvas;
};
